 ...
 *
 * One sheet per piece, and the sheet always reads top-down: title, then the blocking
 * diagram, then the instructions full width beneath it. The diagram earns its place at
 * the top because you check the knitting against it; the instructions do not need a
 * narrow column, so they take the whole width and the page count drops.
 *
...
import { describePlan, MARGIN_MM, type TilePlan } from './tiling';
...
function Sheet({ children }: { children: ReactNode }): JSX.Element {
  return <section className="print-sheet">{children}</section>;
}
  ...
  if (mode === 'templates') {
    ...
    return (
      <div className="printdoc templates" aria-hidden="true">
        ...
        {PIECE_ORDER.map((id, i) => {
          const title = pattern.pieces[i]?.title ?? '';
          const svg = svgFor(id);
          const plan = tilePlanFor?.(id);
          if (!plan) return null;
          return plan.tiles.map((t) => (
            <Sheet key={`${id}-${t.row}-${t.col}`}>
              {/* Exactly HEAD_MM tall and clipped — the plan subtracts this height
                  from every tile, so anything taller pushes the window onto a second
                  sheet. Two short lines only; the assembly notes are on the cover. */}
              <div className="tile-head">
                ...
                {/* Every sheet is identified: tiles get taped, separated, and found
                    again months later, so a lone sheet must say what it belongs to. */}
                <div className="tile-meta">{sizeLabel} · {gaugeLabel}</div>
              </div>
              {/* The window onto the drawing. Fixed physical size, overflow hidden;
                  the SVG inside is shifted so this tile shows its own region. */}
              <div
    ...
    <div className="printdoc" aria-hidden="true">
      ...
      {PIECE_ORDER.map((id, i) => (
        <Sheet key={`diagram-${id}`}>
          ...
          <div className="print-diagram big" dangerouslySetInnerHTML={{ __html: svgFor(id) }} />
        </Sheet>
      ))}
    </div>
  );
